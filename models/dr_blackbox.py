...
class DR_Blackbox( BaseModel ):
    ...
    def observe( self, x_sample, theta ):
        ...
        x_predict = tf.stack( x_predict, axis=-1 ) #在最后一维叠加，因为list x_predict中有4个元素，所以是（?,?,?,4）
        return x_predict
...
class DR_BlackboxPrecisions( DR_Blackbox ):
    ...
    def initialize_neural_states(self, n):
        '''Neural states'''
        inp = Dense(self.n_hidden, activation = tf.nn.relu, name="bb_hidden", input_shape=(n,))   #activation = tf.nn.tanh output_size:50
        act_layer = Dense(4+self.n_latent_species, activation = tf.nn.sigmoid, name="bb_act") #blackbox_growth #output_size:6
        deg_layer = Dense(4+self.n_latent_species, activation = tf.nn.sigmoid, name="bb_deg") #blackbox_degradation #output_size:6
    ...
    def gen_reaction_equations(self, theta: object, treatments: object, dev_1hot: object, condition_on_device: object = True) -> object:
        n_iwae = theta.get_n_samples()
        n_batch = theta.get_n_batch()
        devices = tf.tile( dev_1hot, [n_iwae, 1] ) #(batch_size*n_iwae,1)
        treatments_rep = tf.tile( treatments, [n_iwae,1]) #(batch_size*n_iwae,2)

        Z = []
        for i in range(1,self.n_z+1):
            Z.append( getattr(theta,"z%d"%i))
        Z = tf.stack( Z, axis=2 ) #(batch_size,n_iwae,5) theta.z1到theta.z5的堆砌
        ...
        neural_precisions = NeuralPrecisions(self.nspecies, self.n_hidden_precisions, #self.n_hidden_precisions=25, self.nspecies=4, self.act;self.deg反而起作用了
            inputs = self.n_states + self.n_z + self.n_treatments + self.device_depth, #self.n_states=10, inputs=24
            hidden_activation = tf.nn.relu)

        def reaction_equations( state, t ):
            all_reshaped_state = tf.reshape( state, [n_batch*n_iwae, self.n_states]) #(batch_size,n_iwae,10)-->(batch_size*n_iwae,10)
            ...
            ZZ_states = tf.concat( [ reshaped_state, \
                        ...
                        treatments_rep,\
                        devices], axis=1 )    #(batch_size*n_iwae,6+5+2+7)=(batch_size*n_iwae,20)
            ...
            ZZ_vrs = tf.concat( [ all_reshaped_state, \
                        tf.reshape( Z, [n_batch*n_iwae, self.n_z]), \
                        treatments_rep,\
                        devices], axis=1 ) #(batch_size*n_iwae,10+5+2+7)=(batch_size*n_iwae,24)  for x+h (state+hidden_state)
            vrs = neural_precisions.act(ZZ_vrs) - neural_precisions.deg(ZZ_vrs)*reshaped_var_state #vrs is a 4 dimensinal vector

            return tf.reshape( tf.concat( [states,vrs],1),  [n_batch, n_iwae, self.n_states] ) #reaction_equations的返回值 (batch_size, n_iwae,10)
...
class DR_HierarchicalBlackbox( DR_BlackboxPrecisions ):
    ...
    def gen_reaction_equations( self, theta, treatments, dev_1hot, condition_on_device=True ):
        n_iwae = tf.shape( theta.z1 )[1]  
        n_batch = tf.shape( theta.z1 )[0]
        devices = tf.tile( dev_1hot, [n_iwae, 1] ) #(batch_size*n_iwae,7)
        treatments_rep = tf.tile( treatments, [n_iwae,1])
