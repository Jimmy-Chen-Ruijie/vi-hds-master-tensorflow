...
def constrain_parameter(tf_free_param, free_to_constrained, distribution_name, constrained_name):
    if free_to_constrained == IDENTITY:
        ...
        tf_constrained_param = tf.exp(tf_free_param, name='%s_%s' % (distribution_name, constrained_name)) #tf可以给变量起名字
    else:
...
def build_q_local(PARAMETERS, hidden, devs, conds, verbose, kernel_regularizer, use_bias=True, stop_grad=False, plot_histograms=False):
    ...
    for distribution_name in distribution_descriptions.list_of_params:
        ...
        for free_name, constrained_name, free_to_constrained in zip(
            ...
            hidden_conditioned = tf.concat(to_concat, axis=1) #(batchsize,57) 50+7(hidden_unit+device_depth)
            ...
            tf_constrained_param = constrain_parameter(
                tf_free_param, free_to_constrained, distribution_name, constrained_name) #恒等变换，不用指数变换（对value是否大于0没有要求）
        ...
        new_distribution.assign_free_and_constrained(**params) # *表示无名参数 **表示关键字参数
...
def build_q_constant(PARAMETERS, verbose, stop_grad=False):
    ...
    for distribution_name in distribution_descriptions.list_of_params:
        ...
        for free_name, constrained_name, free_to_constrained, init_free in zip(
            ...
            tf_free_param = tf.Variable(init_free, name='%s.%s'%(distribution_name,free_name), trainable=False) #赋予tf_free_param一个init_free的初值
            params[free_name] = tf_free_param #an OrderedDict,{'value':tf.Variable}
...
class ChainedDistribution(object):
    ...
    def log_prob(self, theta, stop_grad=False):  # wraps log_prob for individual distributions
        ...
        for distribution, sample_value in theta.samples.items(): #sample_value:(batch_size,n_iwae)
            if distribution in self.distributions:
                log_probs.append(self.distributions[distribution].log_prob(sample_value, stop_grad)) #具体样本的值？ self.distributions[distribution].mu = TfNormal.mu -->(batch_size,1)

        # stacking issue?
        if log_probs:
            return tf.reduce_sum(tf.stack(log_probs, -1), -1)#(batch_size,n_iwae,14)-->(batch_size,n_iwae)
        # return 0.0 so we can broadcast empty logprob with another non-empty
        return 0.0

    def monte_carlo_mode(self, theta, tile_it=True):
        # Tself.distributions[distribution]ODO(dacart): use or lose tile_it (in related methods too)
    ...
    def sample(self, list_of_u, verbose, stop_grad=False):
        ...
        #for name, distribution, u in zip(self.distributions.iterkeys(), self.distributions.itervalues(), list_of_u):
            ...
            if name == "dummy":
                pdb.set_trace()
            theta = distribution.sample(list_of_u[:, :, idx], stop_grad) #z1-z5 y1-y2 x1-x5 init_x/rfp/yfp/cfp统统sample with reparametrization
...
class TfNormal(TfCrnDistribution):
    ...
    def log_prob(self, x, stop_grad):
        if stop_grad == True:
            ...
            prec = self.prec
            mu = self.mu
        return -LOG2PI + 0.5 * tf.log(prec + 1e-12) - 0.5 * prec * tf.square(mu - x) #mu:(batch_size,1); x:(batch_size,n_iwae) 采用广播机制， 返回（batch_size,n_iwae）
