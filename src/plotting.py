...
def plot_prediction_summary(procdata, names, times, OBS, PREDICT, PRECISIONS, device_ids, log_ws, predict_style, fixYaxis = False):
    ...
    for iu,device_id in enumerate(unique_devices):
        locs = np.where(device_ids == device_id)[0] #返回索引，哪些数据组对应了device_id
        for idx in range(nplots): #nplots=4
            if ndevices > 1:
                ax = axs[iu,idx] #6行4列，6行对应6个devices(Pcat-Pcat, R100-S32....) 4列对应4个输出频道：OD RFP YFP CFP
            else:
                ax = axs[idx]
            w_mu = np.sum(importance_weights[locs,:,np.newaxis]*PREDICT[locs, :, :, idx], 1) #(34,200,86)-->(34,86)
            w_var =  np.sum(importance_weights[locs,:,np.newaxis]*(PREDICT[locs, :, :, idx]**2 + STD[locs, :, :, idx]**2), 1) - w_mu**2 #(34,200,86)-->(34,86)
            w_std = np.sqrt(w_var)
            ...
            if iu == ndevices-1: ax.set_xlabel('Time (h)') #每到第6行的时候设置x轴的标签
            if iu == 0: ax.set_title(names[idx]) #每到第一行的时候 设置子图上方的标签 (ax即为子图阵列中的某个子图，标号由axs[iu,idx]决定)
            if idx == 0: ax.set_ylabel(procdata.pretty_devices[device_id]) #每到第一列的时候，设置子图阵列中的子图纵坐标标签为device的名称
            pp.tight_layout()
            sns.despine()
