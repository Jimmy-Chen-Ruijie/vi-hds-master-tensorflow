...
class ProcData:
    ...
    def __init__(self, data_settings):
        ...
        self.device_depth = sum([depth(cm.values()) for k, cm in self.component_maps.items()]) #总的设备的个数， 用于创建Multihot vector?
...
class Dataset(object):
    ...
    def create_feed_dict_for_index(self, placeholders, index, beta_val, u_value):
        return {placeholders.x_obs: self.X[index], #(36,86,4) #Training dataset
                placeholders.dev_1hot: self.dev_1hot[index], #(36,7)
                placeholders.conds_obs: self.treatments[index], #(36,2)
                placeholders.beta: beta_val,
                placeholders.u: u_value} #(36,200,4)

    def scale_data(self):
        n_outputs = np.shape(self.X)[2] #总共4个output channel
        if self.procdata.normalize is None:
            self.scales = [np.max(self.X[:, :, i]).astype(np.float32) for i in range(n_outputs)] #每个output channel都找出最大值
        else:
        ...
        for i, scale in enumerate(self.scales):
            ...
            if self.procdata.subtract_background: #[:, np.newaxis]在最后一位添加了一个axis
                self.X[:, :, i] -= np.min(self.X[:, :, i], axis=1)[:, np.newaxis] #每个output channel的每个时间sample的最小值 这样每个时间序列都能为正数
